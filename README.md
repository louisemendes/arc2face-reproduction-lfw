...
> **Execution order matters.** Notebook 01 selects and freezes the 20 evaluation identities and
...
This downloads the LFW dataset, samples 20 identities (fixed seed = 42), generates 5 images per
...
- We keep identities with ≥ 2 images and sample **20** with a fixed seed (42).
- **5 images generated per identity per model** → 100 samples per model.

The 20-identity subset is a compute-budget choice for a single Colab session; it is large
enough to produce a stable distribution of the evaluation metrics.
